...
// raft.Node.RegisterNode (T008's join/bootstrap wiring, cmd/llmctld's
// main.go). A node with no registered address (never joined via the
// api_addr-carrying path, or a registration that has not yet replicated)
// resolves ok=false - Forwarder.forwardToReplicas skips it honestly
// (FR-004), never blocking on it.

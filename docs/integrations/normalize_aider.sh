#!/usr/bin/env bash
# normalize_aider.sh - versioned normalization filter for aider's
...
#   (timestamps, temp paths, ordering) from aider's real run output
...
#
# Usage:
#   aider --message "<fixed prompt>" --yes | bash docs/integrations/normalize_aider.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for aider)
#
# Inputs:
#   aider's raw stdout on this script's stdin. Expected real shape
#   (per its own docs, not yet captured live): aider's unified-diff + chat-history output (file edits plus a token/cost summary line).
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - aider config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/aider/          - fixture pair proving this filter
...
# reverse-engineered from a captured real aider run against a live
...
# aider transcript exists yet to reveal whether, or how, its output

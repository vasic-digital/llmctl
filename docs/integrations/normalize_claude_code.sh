#!/usr/bin/env bash
# normalize_claude_code.sh - versioned normalization filter for Claude Code's
...
#   (timestamps, temp paths, ordering) from Claude Code's real run output
...
#
# Usage:
#   claude -p "<fixed prompt>" | bash docs/integrations/normalize_claude_code.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for Claude Code)
#
# Inputs:
#   Claude Code's raw stdout on this script's stdin. Expected real shape
#   (per its own docs, not yet captured live): Claude Code's transcript output (tool-call trace with session IDs and elapsed-time lines).
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - Claude Code config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/claude_code/          - fixture pair proving this filter
...
# reverse-engineered from a captured real Claude Code run against a live
...
# Claude Code transcript exists yet to reveal whether, or how, its output

#!/usr/bin/env bash
# normalize_cline.sh - versioned normalization filter for Cline's
...
#   (timestamps, temp paths, ordering) from Cline's real run output
...
#
# Usage:
#   cline -p "<fixed prompt>" | bash docs/integrations/normalize_cline.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for Cline)
#
# Inputs:
#   Cline's raw stdout on this script's stdin. Expected real shape
#   (per its own docs, not yet captured live): Cline's CLI transcript output (tool-call trace with session IDs and elapsed-time lines).
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - Cline config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/cline/          - fixture pair proving this filter
...
# reverse-engineered from a captured real Cline run against a live
...
# Cline transcript exists yet to reveal whether, or how, its output

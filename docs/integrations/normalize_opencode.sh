#!/usr/bin/env bash
# normalize_opencode.sh - versioned normalization filter for opencode's
...
#   (timestamps, temp paths, ordering) from opencode's real run output so
#   two runs of the same fixed, seeded prompt (see lib/scheduler.sh's
#   LLMCTL_SEED) can be compared byte-for-byte after filtering.
#
# Usage:
#   opencode run "<fixed prompt>" | bash docs/integrations/normalize_opencode.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for opencode)
#
# Inputs:
#   opencode's raw stdout on this script's stdin.
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - opencode config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/opencode/          - fixture pair proving this filter
...
# reverse-engineered from a captured real opencode run against a live
...
# opencode transcript exists yet to reveal whether, or how, its output

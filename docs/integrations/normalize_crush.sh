#!/usr/bin/env bash
# normalize_crush.sh - versioned normalization filter for crush's
...
#   (timestamps, temp paths, ordering) from crush's real run output
...
#
# Usage:
#   crush run "<fixed prompt>" | bash docs/integrations/normalize_crush.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for crush)
#
# Inputs:
#   crush's raw stdout on this script's stdin. Expected real shape
#   (per its own docs, not yet captured live): crush's terminal-UI transcript (tool-call trace with elapsed-time/token-usage lines).
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - crush config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/crush/          - fixture pair proving this filter
...
# reverse-engineered from a captured real crush run against a live
...
# crush transcript exists yet to reveal whether, or how, its output

#!/usr/bin/env bash
# normalize_pi.sh - versioned normalization filter for pi's
...
#   (timestamps, temp paths, ordering) from pi's real run output
...
#
# Usage:
#   pi -p "<fixed prompt>" | bash docs/integrations/normalize_pi.sh > normalized.txt
#   (run twice, diff the two normalized.txt files - identical means the
#   determinism property holds end-to-end for pi)
#
# Inputs:
#   pi's raw stdout on this script's stdin. Expected real shape
#   (per its own docs, not yet captured live): pi's agentic transcript (tool-call trace with elapsed-time/token-usage lines).
#
# Outputs:
...
#
# Cross-references:
#   docs/integrations.md                          - pi config + install
#   docs/integrations/lib_normalize_common.sh     - shared transform primitives
#   tests/fixtures/agent_output/pi/          - fixture pair proving this filter
...
# reverse-engineered from a captured real pi run against a live
...
# pi transcript exists yet to reveal whether, or how, its output

...
`docs/integrations/normalize_claude_code.sh` is the versioned
normalization filter for Claude Code's full CLI-output artifact (Phase 5
...
by name (timestamps, temp paths, ordering) from Claude Code's real run
...
* Expects Claude Code's raw stdout on stdin; per the script's own header,
  its expected real shape (not yet captured live) is "Claude Code's
  transcript output (tool-call trace with session IDs and elapsed-time
  lines)."

## Usage examples

Pipe a live Claude Code run through the filter twice and diff the
results — an empty diff means determinism holds for Claude Code:

```bash
claude -p "<fixed prompt>" | bash docs/integrations/normalize_claude_code.sh > run1.txt
claude -p "<fixed prompt>" | bash docs/integrations/normalize_claude_code.sh > run2.txt
...
```bash
bash docs/integrations/normalize_claude_code.sh < captured_claude_output.txt
```
...
* **Session/request UUIDs** (8-4-4-4-12 hex) become `<UUID>` via
  `norm_strip_uuids` — directly relevant here since Claude Code's own
  session IDs are exactly this UUID shape per the header's expected-shape
  note.
...
* **Line-ordering non-determinism is explicitly NOT handled**: the
  ...
  captured Claude Code transcript exists yet to reveal whether, or how,
...
* **Synthetic-not-captured field patterns**: the header states plainly
  ...
  Claude Code run against a live llmctl server (that capture is
...
* `tests/test_normalize_agents.sh` — the test that runs this filter (as
  ...
  against `tests/fixtures/agent_output/claude_code/run1.txt` and
  `run2.txt`, asserting both normalize to the same output as each other
  and to the checked-in
  `tests/fixtures/agent_output/claude_code/expected_normalized.txt`.
* `docs/integrations.md` — documents the live-challenge determinism
  ...
  run2.txt` usage pattern, and links to Claude Code's install script
  (`docs/integrations/install_claude_code.sh`, a separate, unrelated
  script that installs the `claude` binary rather than filtering its
  output).

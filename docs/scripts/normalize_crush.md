...
`docs/integrations/normalize_crush.sh` is the versioned normalization
filter for crush's full CLI-output artifact (Phase 5 T030,
...
by name (timestamps, temp paths, ordering) from crush's real run output so
...
* Expects crush's raw stdout on stdin; per the script's own header, its
  expected real shape (not yet captured live) is "crush's terminal-UI
  transcript (tool-call trace with elapsed-time/token-usage lines)."

## Usage examples

Pipe a live crush run through the filter twice and diff the results — an
empty diff means determinism holds for crush:

```bash
crush run "<fixed prompt>" | bash docs/integrations/normalize_crush.sh > run1.txt
crush run "<fixed prompt>" | bash docs/integrations/normalize_crush.sh > run2.txt
...
```bash
bash docs/integrations/normalize_crush.sh < captured_crush_output.txt
```
...
* **Elapsed-time/token-usage reporting** (`(12,345 tokens, 3.2s)`-style and
  ...
  specifically calls out crush's terminal-UI transcript as carrying
...
* **Line-ordering non-determinism is explicitly NOT handled**: the
  ...
  captured crush transcript exists yet to reveal whether, or how, its
...
* **Synthetic-not-captured field patterns**: the header states plainly
  ...
  crush run against a live llmctl server (that capture is release-gating,
...
* `tests/test_normalize_agents.sh` — the test that runs this filter (as
  ...
  against `tests/fixtures/agent_output/crush/run1.txt` and `run2.txt`,
  asserting both normalize to the same output as each other and to the
  checked-in `tests/fixtures/agent_output/crush/expected_normalized.txt`.
* `docs/integrations.md` — documents the live-challenge determinism
  ...
  run2.txt` usage pattern, and links to crush's install script
  (`docs/integrations/install_crush.sh`, a separate, unrelated script that
  installs the `crush` binary via npm/brew/go rather than filtering its
  output).

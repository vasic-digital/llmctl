...
`docs/integrations/normalize_opencode.sh` is the versioned normalization
filter for opencode's full CLI-output artifact (Phase 5 T030,
...
by name (timestamps, temp paths, ordering) from opencode's real run output
...
* Expects opencode's raw stdout on stdin. Unlike its sibling filters, this
  script's header does not give a specific "expected real shape" — the
  Inputs comment reads simply "opencode's raw stdout on this script's
  stdin," with no further description of the anticipated transcript
  structure.

## Usage examples

Pipe a live opencode run through the filter twice and diff the results —
an empty diff means determinism holds for opencode:

```bash
opencode run "<fixed prompt>" | bash docs/integrations/normalize_opencode.sh > run1.txt
opencode run "<fixed prompt>" | bash docs/integrations/normalize_opencode.sh > run2.txt
...
```bash
bash docs/integrations/normalize_opencode.sh < captured_opencode_output.txt
```
...
* **Line-ordering non-determinism is explicitly NOT handled**: the
  ...
  captured opencode transcript exists yet to reveal whether, or how, its
...
* **Synthetic-not-captured field patterns**: the header states plainly
  ...
  opencode run against a live llmctl server (that capture is
...
* Sibling per-agent filters with the identical shape (differing only in
  ...
  `normalize_claude_code.sh`, `normalize_cline.sh`, `normalize_continue.sh`,
  `normalize_crush.sh`, `normalize_pi.sh` (all documented in this same
  batch).
* `tests/test_normalize_agents.sh` — the test that runs this filter (as
  ...
  against `tests/fixtures/agent_output/opencode/run1.txt` and `run2.txt`,
  asserting both normalize to the same output as each other and to the
  checked-in `tests/fixtures/agent_output/opencode/expected_normalized.txt`.
* `docs/integrations.md` — documents the live-challenge determinism
  ...
  run2.txt` usage pattern, and links to opencode's install script
  (`docs/integrations/install_opencode.sh`, a separate, unrelated script
  that installs the `opencode` binary rather than filtering its output —
  also documented in this same batch).

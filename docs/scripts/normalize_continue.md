...
`docs/integrations/normalize_continue.sh` is the versioned normalization
filter for continue.dev's full CLI-output artifact (Phase 5 T030,
...
by name (timestamps, temp paths, ordering) from continue.dev's real run
output so two runs of the same fixed, seeded prompt (see
`lib/scheduler.sh`'s `LLMCTL_SEED`) can be compared byte-for-byte after
filtering.
...
* Expects continue.dev's raw stdout on stdin; per the script's own header,
  its expected real shape (not yet captured live) is "continue.dev's `cn`
  CLI headless output (tool-call trace, `--format json` optionally)." This
  targets continue.dev's standalone CLI (`cn`), the tool documented in
  `docs/integrations.md`'s "continue.dev" section.

## Usage examples

Pipe a live continue.dev run through the filter twice and diff the
results — an empty diff means determinism holds for continue.dev:

```bash
cn -p "<fixed prompt>" | bash docs/integrations/normalize_continue.sh > run1.txt
cn -p "<fixed prompt>" | bash docs/integrations/normalize_continue.sh > run2.txt
...
```bash
bash docs/integrations/normalize_continue.sh < captured_cn_output.txt
```
...
* **`--format json` output is not specially parsed**: the filter is a
  pure text-substitution pipeline regardless of whether `cn`'s optional
  JSON output mode was used — it does not attempt to parse or re-serialize
  JSON, only apply the same four `sed`-based substring substitutions.
...
* **Line-ordering non-determinism is explicitly NOT handled**: the
  ...
  captured continue.dev transcript exists yet to reveal whether, or how,
  its output needs deterministic re-ordering; tracked as a release-gating
...
* **Synthetic-not-captured field patterns**: the header states plainly
  ...
  continue.dev run against a live llmctl server (that capture is
  release-gating, real-GPU work per `docs/quickstart.md`'s live-challenge
  determinism section) — this filter is the mechanism that capture will
  run through, not proof it has already run.
...
* Sibling per-agent filters with the identical shape (differing only in
  ...
  `normalize_claude_code.sh`, `normalize_cline.sh`, `normalize_crush.sh`,
...
* `tests/test_normalize_agents.sh` — the test that runs this filter (as
  ...
  keyed on the `continue` agent name) against
  `tests/fixtures/agent_output/continue/run1.txt` and `run2.txt`,
  asserting both normalize to the same output as each other and to the
  checked-in `tests/fixtures/agent_output/continue/expected_normalized.txt`.
* `docs/integrations.md` — documents the live-challenge determinism
  ...
  run2.txt` usage pattern, and links to continue.dev's install script
  (`docs/integrations/install_continue.sh`, a separate, unrelated script
  that installs the `cn` binary rather than filtering its output). That
  section also notes continue.dev's upstream repository is now
  read-only/archived (acquired by Cursor, June 2026) while the standalone
  `cn` CLI remains fully functional.

...
`docs/integrations/normalize_pi.sh` is the versioned normalization filter
for pi's full CLI-output artifact (Phase 5 T030,
...
by name (timestamps, temp paths, ordering) from pi's real run output so
...
* Expects pi's raw stdout on stdin; per the script's own header, its
  expected real shape (not yet captured live) is "pi's agentic transcript
  (tool-call trace with elapsed-time/token-usage lines)." This is the
  `@earendil-works/pi-coding-agent` CLI (binary `pi`, homepage
  https://pi.dev), not any other "pi"-named tool.

## Usage examples

Pipe a live pi run through the filter twice and diff the results — an
empty diff means determinism holds for pi:

```bash
pi -p "<fixed prompt>" | bash docs/integrations/normalize_pi.sh > run1.txt
pi -p "<fixed prompt>" | bash docs/integrations/normalize_pi.sh > run2.txt
...
```bash
bash docs/integrations/normalize_pi.sh < captured_pi_output.txt
```
...
* **Elapsed-time/token-usage reporting** (`(12,345 tokens, 3.2s)`-style and
  ...
  via `norm_strip_durations` — directly relevant here since the header
  specifically calls out pi's agentic transcript as carrying
  elapsed-time/token-usage lines.
...
* **Line-ordering non-determinism is explicitly NOT handled**: the
  ...
  captured pi transcript exists yet to reveal whether, or how, its output
...
* **Synthetic-not-captured field patterns**: the header states plainly
  ...
  pi run against a live llmctl server (that capture is release-gating,
...
* Sibling per-agent filters with the identical shape (differing only in
  ...
  `normalize_crush.sh`, `normalize_opencode.sh` (all documented in this
  same batch).
* `tests/test_normalize_agents.sh` — the test that runs this filter (as
  ...
  against `tests/fixtures/agent_output/pi/run1.txt` and `run2.txt`,
  asserting both normalize to the same output as each other and to the
  checked-in `tests/fixtures/agent_output/pi/expected_normalized.txt`.
* `docs/integrations.md` — documents the live-challenge determinism
  ...
  run2.txt` usage pattern, and links to pi's install script
  (`docs/integrations/install_pi.sh`, a separate, unrelated script that
  installs the `pi` binary rather than filtering its output — also
